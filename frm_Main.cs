...
namespace ImgONE
{
	...
	public partial class frm_Main : Form
	{
		...
		public frm_Main()
		{
			...
			try {
				if(Convert.ToInt32(Settings.Read("ImgONE", "build")) < Settings.build) {
					File.Delete(Settings.exe_path);
					Settings.Write("ImgONE", "build", Settings.build.ToString());
				}
			} catch (Exception ex) {
				Console.WriteLine("Couldn't delete old ImgONE executable.");
			...
			if(!File.Exists(Settings.ini_path)) {
				DialogResult result = MessageBox.Show(
					"Do you want to run ImgONE at Windows startup?",
					"First time run",
			...
			// Update notification
				...
				DialogResult result = MessageBox.Show(
					"A new version of ImgONE has been released. Would you like to download it?",
					"Update available",
					MessageBoxButtons.YesNo,
					MessageBoxIcon.Question,
					MessageBoxDefaultButton.Button1
